...
- An assessment the outline does not date (**"scheduled by the Registrar", "Date TBA", "during the exam period"**) is shown with that reason and gets **no calendar event** rather than an invented date; a window such as "April 7-30", "Dec. 11-22, 2025" or "written in January" is a window, never its first day, and a schedule row that names the Registrar or the exam period never dates anything
- A course that meets on two days with different lengths ("Mondays (10:30-11:30 AM) and Wednesdays (9:30-11:30 AM)") is **one preselected lecture section with two weekly series**, not two alternative "sections"; the picker only appears when the outline prints section numbers
...
4. **`schedule.extract_slots_and_notes`** + **`pipeline.group_slots`** — lecture / lab / tutorial slots from timetable tables and prose (day letters, dotted times, per-section rows, bulleted "Lectures: / Hours: Tuesdays 9:30-11:30 am, and Thursdays 9:30-10:30 am", a bare "Time: Tuesdays 12:30-2:30 pm and Thursdays 12:30-1:30 pm", and "Section 001: Tuesdays, 1:30pm-4:30pm, SSC 2036" lines under a "Class Location and Time" heading); a component with no day or time becomes a note, never a slot, and a dated "Section 001: Tues Oct 7, 2pm-4pm" exam line is never a weekly slot. Slots of one kind with the same section id (or no id, for lectures) on disjoint weekdays are grouped into one `SectionOption` with several `Meeting`s (Biol 3415G Mon 10:30-11:30 + Wed 9:30-11:30; CS 2211A "Section 001" Tue + Thu), each meeting becoming its own weekly series in the `.ics`; id-less labs and tutorials stay alternative sections.
5. **`assessments.extract_assessments`** — evaluation tables (weight column, continuation rows, footnote digits), inline lists ("Assignment 1 (10%) -- due Oct. 9", "First test: 20% (12 November 2025)"), numeric text tables, then dates for still-undated rows from the weekly schedule, from prose, and from dated bullet lists ("- Assignment 1: … (available: September 28, deadline: October 8, at 11:55 pm)", "- Midterm exam: Weeks 1–6 (Oct 21, 12:30–2:30 pm, in class)", "- Project: Submission deadline: Dec 6, 11:55 pm"). A group row such as "Assignments 17% (three assignments: the first one 5%, and the remaining two, 6% each)" is split into its dated items when the outline lists them; an exam whose date depends on the section ("Section 001: Tues Oct 7 … Section 002: Wed Oct 8") gets the window and a note, never one of the dates; "Chapter 1, Chapter 2 assignments: due Sept 19" lists become one recurring row. An "every Friday from … to …" rule yields to the outline's own date list ("September 12, 19, 26; October 10, 17, 31; and November 14") when there is one, and when the outline states a count that the weekly expansion does not match and lists no dates, the row gets a window and no dates rather than invented ones. An in-class midterm or quiz with no printed time takes the lecture slot's time. A sentence that dates a whole numbered family at once ("The tests are tentatively set to be held on October 2nd, 2025 and October 30th, 2025. Both will be held from 1:30 pm to 3:30 pm") dates "Term Test 1" and "Term Test 2" in order, only when the count of dates equals the count of undated members. A row the evaluation table already leaves to the Registrar, a window or TBA keeps that status: the weekly schedule's "April 7-30 | Final Exam scheduled by the registrar" is not a date. The Engineering template header "Course Outline Fall/Winter 2025" (one year) is read as the Fall term when every date in the outline falls in the Fall and nothing mentions a second term. `dates.DateResolver` turns each date cell into a date + time + status (`exact`, `registrar`, `tba`, `range`, `rule`, `recurring`, `missing`), choosing the year from the term window (Sept–Dec → first year) or the printed weekday.
6. **Rule Resolution** (`rule_resolver.py`, `app.expand_rule_assessments`) — a relative rule such as "Lab Report due 24hrs after each Lab Session" gets an anchor (lab / tutorial / lecture) from its wording; the review page says "Add your lab slot with Add Section and you get one due event per lab", and once that slot is chosen (from the outline or added by hand) the download carries one dated "Lab report N due" event per lab occurrence, the day after each lab (or at the clock time the rule names: "Day of lab at 11:59pm" → 23:59), skipping reading week and capped at a count the outline states ("Labs (Total = 8)", "10 sessions"; the digit in "Term 1 Lab Assignments" is a label, not a count). A "Term 1" / "Term 2" row in a September-to-April course is clipped to its half. The rule is never resolved at upload time, so a printed lab slot and a manually added one behave the same; the stored row keeps its rule, so changing the slot later regenerates the events. Statutory holidays (Thanksgiving Monday) are not modelled: delete that one event.
...
Measured on the labelled corpus (24 real Western outlines with hand-written ground truth, six of them drawn at random, `tests/corpus/`; the PDFs themselves are not committed) after the 2026-09-19 fix round 3:

| metric | result | bar |
|---|---|---|
| course code / name | 24/24 · 23/24 | 90 % |
| term window | 24/24 | 90 % |
| slots recall / precision (one per weekly meeting) | 27/27 · 27/27 | 90 % |
| assessments recall / precision | 119/120 · 119/119 | 95 % |
| weights | 119/119 | 98 % |
| due dates exact (day, month, year) | 61/62 | 95 % |
| no fabricated dates (a Registrar / window / TBA row with *any* date counts as fabricated) | 57/57 | 100 % |
| clean titles | 117/119 | 95 % |
| weight totals | 23/24 | 90 % |

A second corpus test checks the `.ics` itself: the default download of every labelled outline carries one weekly series per lecture meeting. All 42 corpus outlines parse without an exception. The baseline before the fix rounds (commit `de32e96`) was 14 % exact due dates, 0/10 term windows, 27 % slot recall.
